...
(function($) {
	...
	$.fn.jqGrid = function(pin) {
		...
		return this.each(function() {
			...
			imgs = "<span class='s-ico'><span title='Sort Desc' sort='desc' class='ui-grid-ico-sort ui-icon-desc" + idc + " ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-" + dir + "'></span>";
			imgs += "<span title='Sort Asc' sort='asc' class='ui-grid-ico-sort ui-icon-asc" + iac + " ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-" + dir + "'></span></span>";
			if (ts.p.multiSort) {
			...
			$("th", thr).each(function(j) {
				...
				if (!(nm === "cb" || nm === "subgrid" || nm === "rn")) {
					if (ts.p.viewsortcols[2]) {
						$(">div", this).addClass("ui-jqgrid-sortable ").addClass((sort)?"":"cursor-default");
					}
				}
				if (sort) {
					if (ts.p.multiSort) {
...
(function(a) {
	...
	a.fn.fmatter.rowactions = function(h) {
		var g = a(this).closest("tr.jqgrow"), k = g.attr("id"), n = a(this).closest("table.ui-jqgrid-btable").attr("id").replace(/_frozen([^_]*)$/, "$1"), l = a("#" + n), d = l[0], b = d.p, j = b.colModel[a.jgrid.getCellIndex(this)], m = j.frozen ? a("tr#" + k + " td:eq(" + a.jgrid.getCellIndex(this) + ") > div", l) : a(this).parent(), f = {
			extraparam : {}
		}, c = function(p, o) {
	...
	b.jgrid.extend({
		searchGrid : function(c) {
			...
			return this.each(function() {
				...
				if (b("#"+b.jgrid.jqID(f.themodal))[0] !== undefined) {
					...
					c.gbox = "#gbox_" + d;
					if (c.layer) {
						b.jgrid.createModal(f, u, c, "#gview_" + b.jgrid.jqID(t.p.id), b("#gbox_"+b.jgrid.jqID(t.p.id))[0], "#" + b.jgrid.jqID(c.layer), {
							position : "relative"
						})
					} else {
		...
		editGridRow : function(c, d) {
			...
			return this.each(function() {
				...
				var B = e.p.id, y = "FrmGrid_" + B, S = "TblGrid_" + B, v = "#" + b.jgrid.jqID(S), j = {
					themodal : "editmod" + B,
					modalhead : "edithd" + B,
					modalcontent : "editcnt" + B,
					scrollelm : y
				}, C = b.isFunction(a[e.p.id].beforeShowForm) ? a[e.p.id].beforeShowForm : false, K = b.isFunction(a[e.p.id].afterShowForm) ? a[e.p.id].afterShowForm : false, J = b.isFunction(a[e.p.id].beforeInitData) ? a[e.p.id].beforeInitData : false, p = b.isFunction(a[e.p.id].onInitializeForm) ? a[e.p.id].onInitializeForm : false, V = true, F = 1, r = 0, D, A, E;
				y = b.jgrid.jqID(y);
				if (c === "new") {
					c = "_empty";
					E = "add";
					...
					b(ag.p.colModel).each(function(aq) {
						...
						if (an !== "cb" && an !== "subgrid" && this.editable === true && an !== "rn") {
							...
							var ap = b.extend({}, this.editoptions || {}, {
								...
								rowabove : false,
								rowcontent : ""
							}, this.formoptions || {}), ar = parseInt(av.rowpos, 10) || ac + 1, au = parseInt((parseInt(av.colpos, 10) || 1) * 2, 10);
				...
				b.jgrid.viewModal("#" + b.jgrid.jqID(j.themodal), {
					...
					onHide : function(X) {
						b(e).data("formProp", {
							top : parseFloat(b(X.w).css("top")),
							left : parseFloat(b(X.w).css("left")),
							width : b(X.w).width(),
							height : b(X.w).height(),
		...
		viewGridRow : function(c, d) {
			...
			return this.each(function() {
				...
				var s = y.p.id, C = "ViewGrid_" + b.jgrid.jqID(s), u = "ViewTbl_" + b.jgrid.jqID(s), v = "ViewGrid_" + s, o = "ViewTbl_" + s, j = {
					themodal : "viewmod" + s,
					modalhead : "viewhd" + s,
					modalcontent : "viewcnt" + s,
					scrollelm : C
				}, B = b.isFunction(a[y.p.id].beforeInitData) ? a[y.p.id].beforeInitData : false, m = true, h = 1, g = 0;
				if (!d.recreateForm) {
					...
					b(S.p.colModel).each(function(ae) {
						...
						if (I !== "cb" && I !== "subgrid" && I !== "rn" && Z) {
							...
							var ai = b.extend({}, {
								rowabove : false,
								rowcontent : ""
							}, this.formoptions || {}), af = parseInt(ai.rowpos, 10) || W + 1, ah = parseInt((parseInt(ai.colpos, 10) || 1) * 2, 10);
							if (ai.rowabove) {
		...
		delGridRow : function(c, d) {
			...
			return this.each(function() {
				...
				var w = b.isFunction(a[s.p.id].beforeShowForm), j = b.isFunction(a[s.p.id].afterShowForm), u = b.isFunction(a[s.p.id].beforeInitData) ? a[s.p.id].beforeInitData : false, k = s.p.id, m = {}, i = true, g = "DelTbl_" + b.jgrid.jqID(k), r, p, l, n, e = "DelTbl_" + k, f = {
					themodal : "delmod" + k,
					modalhead : "delhd" + k,
					...
					a('td[role="gridcell"]', m).each(function(z) {
						...
						if (v !== "cb" && v !== "subgrid" && v !== "rn") {
							...
							if (s[z].editable === true) {
								...
								var x = a.extend({}, s[z].editoptions || {}, {
									id : c + "_" + v,
									name : v
								});
								if (!s[z].edittype) {
