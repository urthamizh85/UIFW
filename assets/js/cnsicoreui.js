function cnsiCoreUi() {
	var _this = this;
	_this.load_page_id = $("#load_page");
	_this.root = "pages/";
	_this.eleKeypage = $("[data-load-keypage]");
	_this.core = cnsiJS.core;
	_this.historyPage = _this.historyPage || [];
			...
			if (loc_url != "" && loc_keypage != "none") {
				...
				_this.loadPage({
					...
					success : function(data) {
						...
						_this.pageDetails.currentPage = {
							url : loc_url,
							keypage : loc_keypage
						};
						sessionStorage.pageDetails = JSON.stringify(_this.pageDetails);
	...
	_this.datePicker = function(id, options) {
		...
		}).off('focus').on("blur", function(e) {
			var self = $(this);
			self.e = e;
			if ( typeof loc_options.blur === "function") {
				loc_options.blur(self);
			}
		}).on("beforeShowDay", function(e) {
			if ( typeof loc_options.onShow === "function") {
				loc_options.onShow(e);
			}
		});
	...
	_this.pageFontSize = function() {
		...
		$(document).off("click", "[data-font-size]").on("click", "[data-font-size]", function() {
			...
			if (fontType == "plus") {
				...
				fontSize = 140;
				$('body').attr('style', 'font-size:140% !important');
			}
			_this.setPageHeight();
		});
		return this;
	...
	_this.persistPage = function() {
		var keypage = _this.pageDetails.currentPage.keypage, loc_elem = $('[data-keypage="' + keypage + '"]');
		var loc_parent = loc_elem.closest('[data-menu]');
		if (loc_parent.length > 0) {
			if (loc_parent.attr('data-menu') == 'submenu') {
				loc_parent.children('a').trigger('click');
			}
		}
		return this;
	};
